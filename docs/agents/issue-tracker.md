...
- **Link a ticket to its epic**: use GitHub's native sub-issues endpoint — `gh api --method POST repos/<owner>/<repo>/issues/<epic>/sub_issues -F sub_issue_id=<ticket-db-id>` (plural `sub_issues`, and `-F` not `-f` — the ticket's numeric **database id** must be sent as an integer, via `gh api repos/<owner>/<repo>/issues/<n> --jq .id`, not `#number`). GitHub's own REST API is inconsistent here: `POST`/`GET`/`PATCH` on this resource all use the plural `sub_issues`, but `DELETE` (remove a sub-issue) uses the singular `sub_issue` — this isn't a version skew, both forms are current and permanent, so don't "fix" the DELETE form to match. Also write `Part of #<epic>` at the top of the ticket body as a human-readable backup.
